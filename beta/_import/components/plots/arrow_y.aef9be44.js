...
import { color_scale } from "../utils/colors.069c64c0.js";
...
export function arrow_eis(
  ...
  const df_pivoted = aq
    .from(df_filtered)
    .filter((d) => d.year === 2025 || d.year === 2021)
    .groupby("country", "iso3")
    .pivot("year", "obs_value")
    .orderby(aq.desc("2025"))
    .objects();
  const df_sorted = df_pivoted.map((d) => d.country);
  const anchor_early = later_greater ? "end" : "start";
  const anchor_late = later_greater ? "start" : "end";

  return Plot.plot({
    height: (250 * df_sorted.length) / 11,
    marginLeft: 100,
    marginTop: 35,
    marginRight: 50,
    caption:
      df_filtered[0].var_axis === "Global Innovation Index Score"
        ? "Source: WIPO (2025)"
        : "Source: European Commission (2025)",
    color: color_scale,
    x: {
      label: `${df_filtered[0].var_axis} ${year_start}–${year_end}`,
      labelAnchor: "left",
      grid: true,
      axis: "top",
      labelOffset: 35,
    ...
    marks: [
      ...
      Plot.text(
        df_pivoted.filter((d) => d.country === "Switzerland"),
        {
          x: year_start,
          y: "country",
          text: [year_start],
