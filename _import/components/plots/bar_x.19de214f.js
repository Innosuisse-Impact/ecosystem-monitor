...
import { color_scale, countries } from "../utils/colors.069c64c0.js";
import { html } from "../../../_npm/htl@0.3.1/72f4716c.js";
import { style } from "../../../_node/d3@7.9.0/index.e21134d2.js";

export function bar_x(
  data,
  type = "investor",
  source = undefined,
  source_link = undefined
) {
  const df_epo_type = data.filter(
    (d) => d.type === type && countries.includes(d.iso3)
  );
  return Plot.plot({
    caption: html`Source:
    ...
    x: {
      domain: [0, 100],
      label:
        "Percentage of companies with at least one patent in the investment portfolio",
      percent: true,
      ticks: 10,
      axis: "top",
    },
