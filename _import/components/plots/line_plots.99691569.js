...
import { occlusionY } from "../utils/occlusionY.2b6f1d9b.js";
import * as aq from "../../../_node/arquero@7.2.0/index.fbd5f71c.js";
import { color_scale, color_range } from "../utils/colors.069c64c0.js";
...
export function line_oecd(
  ...
  const pct = percent ? " %" : "";
  const label =
    data[0].var_axis === undefined
      ? data[0].unit_name === undefined
        ? data[0].measure_name
        : data[0].measure_name + ", " + data[0].unit_name
      : data[0].var_axis;
  ...
  return Plot.plot({
    caption:
      download_link === undefined
        ...
        : html`Source: <a href="${source_link}" target="_blank">${source}</a> •
            <a href="${download_link}" download>⤓ Download image</a>`,
    marginTop: 30,
    marginRight: 145,
    insetLeft: 7.5,
    color: color_scale,
    ...
    x: {
      ...
      domain:
        data[0].var_axis ==
        "Multifactor productivity (hours based), Index = 2015, Current prices"
          ? [new Date("2015"), new Date("2024")]
          : [new Date("2017"), new Date("2024")],
    },
    y: {
      label: label,
      ticks: 5,
      ...
      Plot.text(
        ...
        occlusionY()(
          Plot.selectLast({
            ...
            text: wrap
              ? (d) =>
                  d.year_date < new Date("2024")
                    ? `${d.country}\n(${d.obs_value.toFixed(decimals)}${pct})`
                    : `${d.country} (${d.obs_value.toFixed(1)} %)`
              : (d) => `${d.country} (${d.obs_value.toFixed(decimals)}${pct})`,
...
export function line_cis(data, measure = "pct_inno", domain_x, color_scale) {
  ...
  const df_plot = data.filter((d) => d.measure === measure); // only for selected countries
...
export function line_rank(
  ...
  return Plot.plot({
    caption: html`Source:
      <a
        href="https://innovationserhebung.ch/innovationsbericht_2023/"
        target="_blank"
        >KOF (2025)</a
      >`,
    height: 200,
        ...
        Plot.pointer({
          ...
          title: (d) =>
            d.obs_value === null
              ? ""
              : `${
                  d.indicator_value_en
                } (${d.date_end.getFullYear()}):\n${d.obs_value.toFixed(1)} %`,
          strokeWidth: 0,
        })
      ),
      Plot.text(
